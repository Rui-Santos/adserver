...
$log = Logger::getLogger('admin_functions.php');
$log->debug("start admin_functions");
...
function quick_publication_report(){
    ...
    // user_group_id is the account_type col in md_uaccounts table
    // map is in md_publications_user_groups 
    $user_group_id = $user_detail['account_type'];
    $log->debug("user_group_id = $user_group_id");
    if ($user_group_id == 1) { // Administrators 
        // show everything
        $log->debug("show everything");
        $sql = "SELECT SUM(total_requests) AS total_requests, SUM(total_impressions) AS total_impressions, SUM(total_clicks) AS total_clicks, publication_id FROM md_reporting WHERE day='$today_day' AND publication_id!='' AND month='$today_month' AND year='$today_year' GROUP BY publication_id";
        $log->debug("sql=$sql");
        $glistres=mysql_query($sql, $repdb);
    }
    else {
        $log->debug("show only publications belong to user_group_id $user_group_id");
        $sql = "SELECT SUM(total_requests) AS total_requests, SUM(total_impressions) AS total_impressions, SUM(total_clicks) AS total_clicks, publication_id FROM md_reporting r, md_publications_user_groups p WHERE r.publication_id = p.inv_id AND p.user_group_id = $user_group_id AND day='$today_day' AND publication_id!='' AND month='$today_month' AND year='$today_year' GROUP BY publication_id";
        $log->debug("sql=$sql");
        $glistres=mysql_query($sql, $repdb);
    }
...
function get_reporting_data($type, $day, $month, $year, $detail){
    ...
    $log->debug("start get_reporting_data");
    if (!MAD_connect_repdb()){
        echo "Could not connect to reporting database. Exiting."; exit;	
    }
    else {
    ...
    if ($type=="publisher"){
        ...
        if ($user_group_id == 1) { // Administrators 
            // show everything
            $sql = "SELECT SUM(total_requests) AS total_requests, SUM(total_impressions) AS total_impressions, SUM(total_clicks) AS total_clicks FROM md_reporting WHERE entry_id>0".$detailquery."".$dayquery."".$monthquery."".$yearquery."";
            $log->debug("sql=$sql");
            $result_repselect=mysql_query($sql, $repdb);
        }
        else {
            // limit view only to that user_group
            $sql = "SELECT SUM(total_requests) AS total_requests, SUM(total_impressions) AS total_impressions, SUM(total_clicks) AS total_clicks FROM md_reporting r, md_publications_user_groups p WHERE r.publication_id = p.inv_id and p.user_group_id=".$user_group_id." and r.entry_id>0".$detailquery."".$dayquery."".$monthquery."".$yearquery."";
            $log->debug("sql=$sql");
            $result_repselect=mysql_query($sql, $repdb);
        }
...
function show_notifications(){
    if (!extension_loaded("SimpleXML")){
        ...
        echo '<p>Adskom has detected that the SimpleXML PHP Module is not installed on this machine. Adskom will not work properly if you do not install this module. For more information on SimpleXML, <a href="http://php.net/manual/book.simplexml.php">click here</a>.</strong></p>';
        echo '</div> <!-- .notify -->';
        echo '</div>';
    }
...
function get_publications($user_id){
    global $maindb;	
    global $log;
    $current_timestamp=time();
    $sql = "select account_type from md_uaccounts where user_id = $user_id";
    $log->debug("sql=$sql");
    $useracctype = mysql_get_var($sql);
    if ($useracctype == 1) {
        $sql = "select * from md_publications ORDER BY inv_id DESC";
    }
    else {
        $sql = "select p.* from md_publications p where p.inv_id in (select inv_id from md_publications_user_groups where user_group_id = $useracctype)";
    }
    $log->debug("sql=$sql");
    $usrres=mysql_query($sql, $maindb);
...
function get_publication_dropdown($selected, $user_id){
    global $maindb;	
    global $log;
    $sql = "select account_type from md_uaccounts where user_id = $user_id";
    $log->debug("sql=$sql");
...
function get_publication_dropdown_report($selected, $user_id=1){
    global $maindb;	
    global $log;
    $sql = "select account_type from md_uaccounts where user_id = $user_id";
    $log->debug("sql=$sql");
        ...
        if ($data['creative_type']==1 && $no_creative!=1){
            ...
            if (getconfig_var('default_creative_server')>1){
                ...
                if ($creative_server_detail['entry_id']<1){
                    global $errormessage;
                    $errormessage='The default creative server does not seem to exist. Please change your creative server in your Adskom control panel under Configuration>Creative Servers';
                    global $editdata;
                    $editdata=$data;
                    return false;
                }
...
function do_create($type, $data, $detail){
    ...
    if ($type=='campaign'){
        ...
        if ($data['campaign_type']!='network'){
            ...
            // Define Image Sizes
            $data['custom_creative_width']  = $ad_sizes[$data['creative_format']]['width'];
            $data['custom_creative_height'] = $ad_sizes[$data['creative_format']]['height'];
            // End Define Image Sizes
            ...
            if ($data['creative_type']==1){
                ...
                if (getconfig_var('default_creative_server')==1){
                    ...
                    global $errormessage;
                    $errormessage='5 Creative could not be uploaded. Please check if your creative directory is writeable ('.MAD_CREATIVE_DIR.') and that you have uploaded a valid image file.';
                    global $editdata;
                    $editdata=$data;
                    return false;
                    }
                    ...
                    global $errormessage;
                    $errormessage='6 Creative could not be uploaded. Please check if your creative directory is writeable ('.MAD_CREATIVE_DIR.') and that you have uploaded a valid image file.';
                    global $editdata;
                    $editdata=$data;
                    return false;
    ...
    if ($type=='placement'){
        ...
        $data['zone_width']  = $ad_sizes[$data['zone_size']]['width'];
        $data['zone_height'] = $ad_sizes[$data['zone_size']]['height'];
        /*
    ...
    if ($type=='publication'){
        ...
        $sql = "select account_type from md_uaccounts where user_id = ". $user_detail['user_id'];
        $useracctype = mysql_get_var($sql);

        mysql_query("INSERT INTO md_publications_user_groups (inv_id, user_group_id) VALUES ($new_publication_id, $useracctype)", $maindb);
...
function get_pubid_table($networkid, $publication_id){
    ...
    echo '<tr class="gradeA">
        <td valign="middle"><div style="margin-top:4px;" align="center"><em>Default</em> <a href="#" onclick="$.modal ({title: \'Publisher ID Types\', html: \'<div style=width:500px;;>Adskom requests ads directly from the respective ad network over an API connection. Since Adskom is not a middle-man, you need to have an account at the ad network you would like to use in order to start driving revenue through it. Your ad network earnings will be paid directly by the ad network you choose to use.<br><br><h2>Setting up Publisher IDs</h2>Each network requires a different set of Publisher identifers. Inside Adskom, you can add Publisher IDs on a Global Default Level, Publication Level and Placement Level. This means that you can either use just one single default Publisher ID for requesting ads from an ad network, or use different Publisher IDs depending on what Publication or Placement the ad is being shown in.<br><br><h2>Where can I find my Publisher ID?</h2>This depends on the network you are using. Usually, your network will show your Publisher ID when you add a site/app in your network account. If you are unsure about where you can find Publisher IDs, please contact the ad network you are using for assistance.<br><br><h2>How can I add Publisher IDs on a placement level?</h2>You can add/modify your Publisher IDs on a Placement Level by clicking on the small + link next to the name of your Publication. If there are no Publisher IDs provided for a single placement, Adskom will automatically use the Publication or Network Default Publisher IDs for the ad request.\'});">Info</a></div></td>';
    if (!empty($network_detail['network_d1_name'])){
